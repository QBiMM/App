...
function mapReplace(blueprint, map2, interpolation) {
  ...
  return blueprint.replace(mapRegExp, (match2) => {
    if (interpolation) return String(map2[match2.replace(/\{\{\s*|\s*\}\}/g, "")]);
    return String(map2[match2]);
  });
}
...
function createLocalizer(localization) {
  return (resourceName, key, defaultValue) => {
    ...
    const resource2 = localization?.values?.[resourceName];
    if (!resource2) return defaultValue;
    return resource2[key] || defaultValue;
  };
}
...
function interpolate2(text, params) {
...
var _LocalizationService = class _LocalizationService {
  ...
  initLocalizationValues() {
    ...
    combineLatest([legacyResources$, remoteLocalizations$, uiLocalizations$]).pipe(map(([legacy, resource2, local]) => {
      if (!resource2) {
        return;
      }
      const remote = combineLegacyandNewResources(legacy || {}, resource2);
      if (remote) {
        if (!local) {
        ...
        Object.entries(remote).forEach((entry) => {
          ...
          let resource3 = local?.get(resourceName) || {};
          resource3 = __spreadValues(__spreadValues({}, resource3), remoteTexts);
          local?.set(resourceName, resource3);
        });
      }
      return local;
  ...
  addLocalization(localizations) {
    ...
    localizations.forEach((loc) => {
      ...
      loc.resources.forEach((res) => {
        let resource2 = cultureMap.get(res.resourceName) || {};
        resource2 = __spreadValues(__spreadValues({}, resource2), res.texts);
        cultureMap.set(res.resourceName, resource2);
      });
  ...
  getLocalization(state, key, ...interpolateParams) {
    ...
    if (localization) localization = interpolate2(localization, interpolateParams);
...
function mergeResourcesWithBaseResource(resource2) {
  const entities = Object.keys(resource2).map((key) => {
    const newValue = recursivelyMergeBaseResources(key, resource2);
    return [key, newValue];
  });
...
function combineLegacyandNewResources(legacy, resource2) {
  const mergedResource = mergeResourcesWithBaseResource(resource2);
...
_BaseCoreModule.ɵinj = ɵɵdefineInjector({
  ...
  imports: [CommonModule, FormsModule, ReactiveFormsModule, RouterModule, NgxValidateCoreModule, DynamicLayoutComponent, ReplaceableRouteContainerComponent, RouterOutletComponent, CommonModule, FormsModule, ReactiveFormsModule, RouterModule, NgxValidateCoreModule]
});
...
@angular/core/fesm2022/rxjs-interop.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-DOR275RM.js.map
